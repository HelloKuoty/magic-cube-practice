...
界面按 5 个分区组织(顶部导航):**今日 · 学习 · 训练 · 实战 · 公式库**;进入分区后再选具体模式。

**◎ 今日**(仪表盘):到期复习数、当前公式、学习路径(查公式→看演示→跟练→复习)与快捷入口。

| 分区 | 模式 | 作用 |
| --- | --- | --- |
| **学习** | 演示 | 选一个公式,魔方逐步演示每一步,可单步 / 播放 / 调速;空格=下一步。开「先做逆操作铺场」后正向播放会复原。 |
| 学习 | 符号 | 点 `R` / `U'` / `M` 等符号,魔方当场演示这一步,悬停高亮对应的层。 |
| 学习 | 手法 | 教学(基础手法字典 + 概念)+ 练习(Sexy/锤子/小鱼等触发的 3D 演示、循环跟练、逐步分解)。 |
| **训练** | 识别 | 闪识别图 → 回想公式 → 看答案自评,按**间隔重复(FSRS-4.5)**优先推最不熟的;OLL/PLL/F2L 可选,键盘 1-4 评分,到期数显示在「今日」上。 |
| 训练 | 跟练 | 看着目标公式按出每一步,判对错、计时、算正确率,记录到后端。支持「盲练」。 |
| 训练 | 读谱 | 看打乱谱/还原谱,像看谱弹琴一样练「看符号→手上执行」,可 3D 单步;空格=下一步。 |
| 训练 | 十字 | 给随机打乱,自己解底面十字,再看**最优解**(PDB 求解器,≤8 步)对照,可 3D 单步。 |
| 训练 | 计时 | 按住空格预备→松开计时→空格停;best / ao5 / ao12 / 平均,历史可 +2 / DNF,存本地。 |
| **实战** | 自由玩 | 可拖动转层的 3D 魔方:拖一个面转那层、拖背景转视角;打乱/复位、计步计时、复原庆祝。F/U/R 面标 +「面对我的就是 F面」视角相对记号。 |
| 实战 | 还原助手 | ①涂最后一层 → 2-look 解法;②**输入整方打乱 → 十字/F2L/OLL/PLL 全程分步解法**,可 3D 单步。 |
...
**校验脚本**:`node scripts/verify-algs.mjs`。也可以在根目录运行 `npm test`,一次跑完公式校验、最后一层求解器随机测试和前端构建。它用魔方坐标模型逐条验证:
...
| `frontend/src/cube/srs.js` | 间隔重复(FSRS-4.5),识别训练的排期与掌握度。后续方向见 [ROADMAP.md](ROADMAP.md) |
